...
//    });
    ...
    app.get("/api/audits/:auditId/generate", acl.hasPermission('audits:read'), function(req, res) {
        ...
        .then(async audit => {
            ...
            const safeName = encodeURIComponent(audit.name.replace(/[\/\\:*?"<>|]/g, ""));
            const encodedSafeName = encodeURI(decodeURIComponent(safeName));
            console.log(`Имя файла: ${safeName}`);

            // Подготовка к отправке файла на сервер Python
            const FormData = require('form-data');
            const formData = new FormData();

            // Добавляем файл в formData для отправки
            formData.append('file', new Buffer.from(reportDoc), `${safeName}.docx`);

            try {
                // Отправляем файл на Python сервер для обработки
                const response = await axios.post('http://172.17.0.1:5000/edit_docx', formData, {
                    headers: formData.getHeaders(),
                    responseType: 'arraybuffer'  // Ожидаем файл как бинарные данные
                });

                // Получаем измененный файл от Python
